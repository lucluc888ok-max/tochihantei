...
class PropertyDataSchema(BaseModel):
    ...
    market_price_per_tsubo: float = Field(..., description="物件周辺相場の参考坪単価（※AI側で一般的な相場を推測するか、記載がなければ2350000等を返却）")
...
def extract_property_data_with_llm(pdf_bytes: bytes) -> PropertyDataSchema:
    ...
    if not api_key:
        ...
        return PropertyDataSchema(
            ...
            market_price_per_tsubo=2354600
        )
    ...
    try:
        ...
        return PropertyDataSchema(
            ...
            market_price_per_tsubo=2000000
        )
